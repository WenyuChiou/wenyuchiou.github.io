import { BehaviorComparisonLab } from "./features/behavior-lab/component.jsx";
      ...
      <div className="flagship-list">{F.items.map((item, index) => {
        ...
        <details className={`flagship-entry case-${role}`} name="selected-work-projects" key={item.slug} open={index === 0}>
          ...
          <div className="flagship-panel">
            ...
            {item.slug === "human-grounded-llm-evaluation" && <SmartLink className="text-link flagship-case-link" href="/work/human-grounded-llm-evaluation/#behavior-comparison" locale={locale}>{locale === "zh-TW" ? "比較模型行為" : "Compare model behavior"}<ArrowRight aria-hidden="true" size={16} /></SmartLink>}
          </div>
        </details>
      );})}</div>
...
function PathwayExplorer({ locale }) {
  return <BehaviorComparisonLab locale={locale} />;
}
...
function CaseInteraction({ type, content, locale }) {
  if (type === "pathways") return <PathwayExplorer locale={locale} />;
