...
await esbuild.build({ entryPoints: ["features/behavior-lab/styles.css"], bundle: true, minify: true, outfile: "assets/behavior-lab.css" });
const behaviorCssHash = createHash("sha256").update(readFileSync("assets/behavior-lab.css")).digest("hex").slice(0, 8);
...
for (const page of generatedPages) {
  ...
  writeFileSync(page, html.includes('id="behavior-comparison"') ? html.replace("</head>", `<link rel="stylesheet" href="/assets/behavior-lab.css?v=${behaviorCssHash}"></head>`) : html);
}
